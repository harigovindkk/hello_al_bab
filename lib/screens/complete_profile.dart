...
class _SignUpPageState extends State<CompleteProfile> {
  ...
  bool profileremoved = false,
      ...
      _isLoading = false, countryCodeSelected=false;
  ...
  void updateProfile() async {
    try {
      ...
      FirebaseFirestore.instance
        ...
        "phone": countryCodeSelected? code.toString() + " " + phonecontroller.text : userDetail!.phone.substring(0, userDetail!.phone.indexOf(' '))+phonecontroller.text,
        'profilePicture': profilechanged
            ? (profileremoved ? "" : profileUrl)
            : userDetail!.profilePicture,
      }).whenComplete(() {
        ScaffoldMessenger.of(context).showSnackBar(customSnackBar(
            "Profile  Updated Successfully", Icons.check));
             setState(() {
        _isLoading = false;
      });
      }).catchError((error) =>
              //-------------------------------------------
              ScaffoldMessenger.of(context).showSnackBar(customSnackBar(
                  "Failed to update profile: ${error}", Icons.warning_amber_rounded)));
    } catch (e) {
      print("Error");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      backgroundColor: Colors.white,
      body: _isLoading?Center(child: CircularProgressIndicator(color: primary,)): SingleChildScrollView(
        child: Column(
                      ...
                      profilechanged
                          ...
                          : (profileremoved
                              ...
                              : ((userDetail!.profilePicture.isEmpty)
                                  ...
                                  : Image.network(userDetail!.profilePicture)))
                    ],
                  ),
                ),
              ),
